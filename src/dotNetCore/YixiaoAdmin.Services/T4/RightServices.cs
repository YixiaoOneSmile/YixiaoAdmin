...
namespace YixiaoAdmin.Services
{
    ...
    public partial class  RightServices:BaseServices<Right>, IRightServices
    {
        ...
        public RightServices(IRightRepository RightRepository)
        {
            this._RightRepository = RightRepository;
            base.baseRepository = _RightRepository;
        }
        public async Task<PagesResponse> QueryPages(QueryPageModel queryPageModel)
        {
            ...
            pagesResponse.Success((await _RightRepository.Query(whereExpression, queryPageModel.Orderby, queryPageModel.CurrentPage, queryPageModel.PageNumber)).ToList());
